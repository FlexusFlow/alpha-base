...
- **Anti-Bot Browser Fingerprint for Scraper** — Playwright scraper uses default headless Chrome user-agent (contains `HeadlessChrome`), which triggers bot detection on sites like Medium (Cloudflare security verification page). Fix: set a realistic Chrome user-agent on the browser context and add a 2-second post-load delay for JS rendering, matching the approach in `medium-legal-scraper`. Affects `backend/app/services/article_scraper.py`.
- **Documentation Site Scraping (ALP-008)** — Add ability to scrape multi-page documentation sites by providing a single entry URL. The system crawls the documentation structure (sidebar, TOC, internal links), discovers all pages within the URL scope (up to 100 pages), previews them for user confirmation, then bulk-scrapes and indexes them into the vector store as a unified collection. Extends the existing article scraping pipeline (ZIP-003) to handle structured, multi-page content. See spec: `specs/ALP-008-doc-scraping/spec.md`
