...
```bash
ZIPTRADER_API_URL=https://your-domain.com
ZIPTRADER_API_KEY=zt_your_api_key_here
```
...
API_BASE_URL = os.getenv('ZIPTRADER_API_URL')
API_KEY = os.getenv('ZIPTRADER_API_KEY')
```
...
const API_BASE_URL = process.env.ZIPTRADER_API_URL;
const API_KEY = process.env.ZIPTRADER_API_KEY;
```
...
# Test 1: Simple query
curl -X POST https://your-domain.com/v1/api/public/query \
  -H "Authorization: Bearer ${ZIPTRADER_API_KEY}" \
  -H "Content-Type: application/json" \
